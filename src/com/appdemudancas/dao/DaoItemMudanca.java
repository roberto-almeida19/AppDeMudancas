...
public class DaoItemMudanca implements IDaoItemMudanca {
	...
	private List<ItemMudanca> resultSetParaList(ResultSet resultado) throws SQLException {
		...
		while (resultado.next()) {
			...
			auxItemMudanca.setE_fragil(
					"s".equals(resultado.getString("altura")) ? true : false);
	...
	private ResultSet gerarResultSet(ItemMudanca itemMudanca) throws SQLException {
		...
		String query = "SELECT * FROM item_da_mudanca WHERE "
				...
				+ "item_da_mudanca.altura=? OR "
				+ "item_da_mudanca.largura=? OR "
				+ "item_da_mudanca.comprimento=? OR "
				+ "item_da_mudanca.peso=? OR "
				+ "item_da_mudanca.e_fragil=? OR"
				+ "item_da_mudanca.MudancaCodigo_mudanca=? OR"
				+ "item_da_mudanca.codigo_item=?;";
